#!/usr/bin/env python
"""
Dual-joystick teleoperation for SO-101 robot arm.

Receives virtual joystick data from a mobile web UI via WebSocket.
The server integrates velocities into an EE target pose, clamps to
workspace bounds, solves analytical IK, and returns joint angles.

Supports both MuJoCo simulation and real robot hardware.

Usage:
    # MuJoCo simulation
    python examples/webphone_to_so101/teleop_webphone.py --mode mujoco

    # Real robot
    python examples/webphone_to_so101/teleop_webphone.py --mode real
"""

import math
import sys
import time
from pathlib import Path

import numpy as np

# ── CLI args ──
_MODE = "mujoco"
_HOST = "0.0.0.0"
_PORT = 4443
for i, arg in enumerate(sys.argv):
    if arg == "--mode" and i + 1 < len(sys.argv):
        _MODE = sys.argv[i + 1]
    elif arg == "--host" and i + 1 < len(sys.argv):
        _HOST = sys.argv[i + 1]
    elif arg == "--port" and i + 1 < len(sys.argv):
        _PORT = int(sys.argv[i + 1])

USE_MUJOCO = _MODE == "mujoco"
USE_REAL = _MODE == "real"

# ── Paths ──
SCRIPT_DIR = Path(__file__).resolve().parent

# ── Imports ──
from web_server import WebPhoneServer

from lerobot.utils.robot_utils import precise_sleep

if USE_MUJOCO:
    from gym_so101.envs.so101_gym_env import (
        ALL_JOINT_NAMES,
        SO101GymEnv,
    )
    from gym_so101.wrappers.viewer_wrapper import PassiveViewerWrapper

# ── Config ──
FPS = 30
DEBUG_PRINT_INTERVAL = 15

# SO-101 joint names (5-DOF arm + gripper)
ARM_JOINT_NAMES_LIST = [
    "shoulder_pan", "shoulder_lift", "elbow_flex", "wrist_flex", "wrist_roll"
]
ALL_JOINT_NAMES_LIST = ARM_JOINT_NAMES_LIST + ["gripper"]

if USE_MUJOCO:
    ARM_JOINT_NAMES_LIST = list(ALL_JOINT_NAMES[:5])
    ALL_JOINT_NAMES_LIST = list(ALL_JOINT_NAMES)

# ── Helpers ──

def joint_action_to_mujoco_ctrl(joint_action: dict) -> np.ndarray:
    """Convert joint action dict (degrees) to MuJoCo control array (radians).

    - Arm joints: degrees → radians
    - wrist_flex: negated (MuJoCo joint direction is opposite to DH convention)
    - gripper: [0,1] normalized → radians in MuJoCo joint range
    """
    ctrl = np.zeros(len(ALL_JOINT_NAMES_LIST), dtype=np.float32)
    for i, name in enumerate(ALL_JOINT_NAMES_LIST):
        val = float(joint_action.get(f"{name}.pos", 0.0))
        if name == "gripper":
            # Map [0,1] normalized → [0, 1.74533] rad (gripper open range)
            ctrl[i] = val * 1.74533
        elif name == "wrist_flex":
            # Negate: MuJoCo wrist_flex axis is opposite to DH IK convention
            ctrl[i] = math.radians(-val)
        else:
            ctrl[i] = math.radians(val)
    return ctrl


def load_joint_limits_from_mujoco(env, margin_pct: float = 0.25) -> dict:
    """Load joint limits from MuJoCo model (radians). Gripper gets [0,1] normalized."""
    limits = {}
    model = env.unwrapped.model
    for name in ALL_JOINT_NAMES_LIST:
        if name == "gripper":
            limits[name] = (0.0, 1.0)
            continue
        jid = model.joint(name).id
        low = model.jnt_range[jid][0]
        high = model.jnt_range[jid][1]
        margin = (high - low) * margin_pct * 0.5
        # Convert to degrees for clipping server output
        limits[name] = (math.degrees(low + margin), math.degrees(high - margin))
    return limits


def clip_joint_action(action: dict, limits: dict) -> dict:
    """Clamp joint positions to limits."""
    result = {}
    for k, v in action.items():
        if isinstance(k, str) and k.endswith(".pos"):
            name = k.removesuffix(".pos")
            if name in limits:
                lo, hi = limits[name]
                result[k] = max(lo, min(hi, float(v)))
            else:
                result[k] = v
        else:
            result[k] = v
    return result


# ── Main ──

def main():
    print(f"\n{'=' * 60}")
    print(f"  Dual-Joystick Teleop — SO-101 Arm Control")
    print(f"  Mode: {'MuJoCo Simulation' if USE_MUJOCO else 'Real Robot'}")
    print(f"{'=' * 60}\n")

    # ── Environment ──
    if USE_MUJOCO:
        env = SO101GymEnv(render_mode="human", control_dt=1.0 / FPS, physics_dt=0.002)
        env = PassiveViewerWrapper(env)
    else:
        env = None

    # ── Web server ──
    server = WebPhoneServer(host=_HOST, port=_PORT)
    server.connect()

    # ── Joint limits ──
    if USE_MUJOCO:
        joint_limits = load_joint_limits_from_mujoco(env, 0.25)
    else:
        joint_limits = {
            "shoulder_pan": (-150, 150), "shoulder_lift": (-120, 120),
            "elbow_flex": (-120, 120), "wrist_flex": (-120, 120),
            "wrist_roll": (-180, 180), "gripper": (0, 1),
        }

    if USE_MUJOCO:
        env.reset()

    # Wait for client
    print("Waiting for phone/tablet to connect...")
    print(f"Open https://<IP>:{_PORT} or scan QR code\n")
    while not server.is_connected:
        time.sleep(0.5)
    print("Client connected! Use the joysticks to control the arm.\n")

    # ── Main loop ──
    frame = 0
    t_last_report = time.perf_counter()
    _last_valid_ctrl = np.zeros(len(ALL_JOINT_NAMES_LIST), dtype=np.float32)
    ctrl = _last_valid_ctrl.copy()

    try:
        while True:
            t0 = time.perf_counter()

            # ── Teleop action (already IK-solved by server) ──
            joint_action = server.get_action()

            enabled = server.enabled and server.is_connected

            if enabled:
                joint_action = clip_joint_action(joint_action, joint_limits)

                if USE_MUJOCO:
                    ctrl = joint_action_to_mujoco_ctrl(joint_action)
                    env.step(ctrl)
                    _last_valid_ctrl = ctrl.copy()
                else:
                    # Real robot: send joint angles (degrees) to motor driver
                    ctrl = np.array([
                        float(joint_action.get(f"{n}.pos", 0))
                        for n in ALL_JOINT_NAMES_LIST
                    ])
                    _last_valid_ctrl = ctrl.copy()

            # ── Debug print ──
            frame += 1
            if frame % DEBUG_PRINT_INTERVAL == 0:
                now = time.perf_counter()
                elapsed = now - t_last_report
                t_last_report = now

                status = "ON " if enabled else "OFF"

                ja = joint_action if isinstance(joint_action, dict) else {}
                sp = ja.get("shoulder_pan.pos", 0)
                sl = ja.get("shoulder_lift.pos", 0)
                ef = ja.get("elbow_flex.pos", 0)
                wf = ja.get("wrist_flex.pos", 0)
                wr = ja.get("wrist_roll.pos", 0)
                gr = ja.get("gripper.pos", 0)

                print(
                    f"\n{'═' * 60}\n"
                    f"  Frame {frame:>5d}  │  {DEBUG_PRINT_INTERVAL / max(elapsed, 0.001):4.0f} FPS  │  STATUS: {status}\n"
                    f"{'─' * 60}\n"
                    f"  Joints (°):       pan={sp:+8.2f}  lift={sl:+8.2f}  elbow={ef:+8.2f}  "
                    f"wflex={wf:+8.2f}  wroll={wr:+8.2f}  grip={gr:+.2f}\n"
                    f"{'═' * 60}"
                )

            precise_sleep(max(1.0 / FPS - (time.perf_counter() - t0), 0.0))

    except KeyboardInterrupt:
        print("\nShutting down...")
    finally:
        if USE_MUJOCO:
            env.close()
        server.disconnect()


if __name__ == "__main__":
    main()
